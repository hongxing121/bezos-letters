#!/usr/bin/env python3
"""
Build a static HTML site from a Bezos shareholder letters Obsidian vault.
...
def build_sidebar_html(files, current_stem=""):
    ...
    # Sort letters by year ascending
    groups["letters"].sort(key=lambda x: x["stem"])
    for cat in ["concepts", "companies", "people", "index-pages"]:
...
def wrap_page(title, body_html, files, current_stem="", right_html="", wide=False, page_type=""):
    ...
    if wide:
        ...
        main_inner = f'<div class="main-content article">{body_html}</div>{right_html}'
    else:
        # Article without backlinks
        main_class = "main"
        main_inner = f'<div class="article">{body_html}</div>'
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="/index.html" class="logo">贝佐斯知识库</a></div>
...
def build_homepage(files, link_map, ref_counts):
    ...
    people_data = {
        "贝佐斯":           ("贝", "创始人、CEO（1994—2021）"),
        "安迪·贾西":        ("贾", "AWS 缔造者、接班人"),
        "杰夫·威尔克":       ("威", "物流帝国的缔造者"),
        "本杰明·格雷厄姆":  ("格", "长期主义的思想源头"),
    }
...
<section class="hero-section">
  <div class="hero-eyebrow">Jeff Bezos · Shareholder Letters</div>
  <h1 class="hero-title">贝佐斯致股东信<span class="gold">知识库</span></h1>
  <p class="hero-sub"><b>24 年</b>经营智慧，<b>24 封</b>亲笔信件，<b>{total_links}+</b> 条交叉链接<br>
  每个概念、每项业务、每个人物——都能一键溯源到原文</p>
</section>

<div class="stats-row">
  <a href="/index-pages/贝佐斯致股东信总览.html" class="stat-item"><div class="stat-num">24</div><div class="stat-label">封股东信</div></a>
  <a href="/index-pages/核心思想索引.html" class="stat-item"><div class="stat-num">{len(concept_files)}</div><div class="stat-label">核心概念</div></a>
  <a href="/index-pages/产品索引.html" class="stat-item"><div class="stat-num">{len(company_files)}</div><div class="stat-label">项产品</div></a>
  <a href="/index-pages/人物索引.html" class="stat-item"><div class="stat-num">{len(people_files)}</div><div class="stat-label">关键人物</div></a>
</div>

<div class="main-inner">
<div class="nav-cards">
  <a href="/index-pages/贝佐斯致股东信总览.html" class="nav-card">
    <span class="nav-card-icon">📬</span>
    <div class="nav-card-title">致股东信总览</div>
    <div class="nav-card-sub">1997 — 2020 完整收录</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/核心思想索引.html" class="nav-card">
    <span class="nav-card-icon">💡</span>
    <div class="nav-card-title">核心思想索引</div>
    <div class="nav-card-sub">{len(concept_files)} 个核心概念解析</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/产品索引.html" class="nav-card">
    <span class="nav-card-icon">🏢</span>
    <div class="nav-card-title">产品索引</div>
    <div class="nav-card-sub">{len(company_files)} 项亚马逊产品档案</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="/index-pages/人物索引.html" class="nav-card">
    <span class="nav-card-icon">👤</span>
    <div class="nav-card-title">人物索引</div>
    <div class="nav-card-sub">{len(people_files)} 位关键人物传记</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
</div>
